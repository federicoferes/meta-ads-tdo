...
import { getSupabase, ACCOUNT_ID, CACHE_TTL_HOURS } from '@/lib/supabase'
...
export async function GET(req: NextRequest) {
  ...
  const campaigns = (cvJson.data ?? []).map((c: Record<string, unknown>) => {
    ...
    return {
      id: c.campaign_id, name: c.campaign_name, objective,
      impressions: Number(c.impressions ?? 0), reach: Number(c.reach ?? 0),
      clicks: Number(c.clicks ?? 0), spend,
      cpm: Number(c.cpm ?? 0), ctr: Number(c.ctr ?? 0), frequency: Number(c.frequency ?? 0),
      resultLabel: result.label, results: result.value, cpr: result.cpr,
      isMsgs: result.isMsgs, isLead: result.isLead,
  ...
  const trend = (tvJson.data ?? []).map((d: Record<string, unknown>) => {
    ...
    return {
      date: d.date_start as string, spend: Number(d.spend ?? 0),
      impressions: Number(d.impressions ?? 0),
      msgsStarted: sumActions(da, MSGS_STARTED), leadsForm: sumActions(da, LEADS_FORM),
    }
  })
  ...
  const platforms = (pvJson.data ?? [])
    ...
    .map((p: Record<string, unknown>) => ({
      platform: p.publisher_platform, spend: Number(p.spend ?? 0),
      impressions: Number(p.impressions ?? 0), clicks: Number(p.clicks ?? 0),
    }))
  ...
  const overview = {
    spend: totalSpend, impressions: Number(o.impressions ?? 0), reach: Number(o.reach ?? 0),
    clicks, pageViews, msgsStarted, msgsConnected, leadsForm,
    totalLeads: msgsStarted + leadsForm,
    cpm: Number(o.cpm ?? 0), ctr: Number(o.ctr ?? 0), frequency: Number(o.frequency ?? 0),
    cpmMsg:  msgsStarted > 0 ? totalSpend / msgsStarted : null,
    cpmLead: leadsForm   > 0 ? totalSpend / leadsForm   : null,
  }

  const prev = {
    spend: prevSpend, impressions: Number(pv.impressions ?? 0), reach: Number(pv.reach ?? 0),
    msgsStarted: prevMsgs, leadsForm: prevLeads,
    cpm: Number(pv.cpm ?? 0), ctr: Number(pv.ctr ?? 0), frequency: Number(pv.frequency ?? 0),
    cpmMsg:  prevMsgs  > 0 ? prevSpend / prevMsgs  : null,
    cpmLead: prevLeads > 0 ? prevSpend / prevLeads : null,
  }

  const payload = { since, until, prevSince, prevUntil, overview, prev, byObjective, campaigns, trend, platforms }

  // ── 3. Save to cache (fire and forget) ───────────────────────────────────
  saveCache(since, until, prevSince, prevUntil, payload).catch(() => {})
...
async function readCache(since: string, until: string) {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const sb  = getSupabase() as any
    const ttl = new Date(Date.now() - CACHE_TTL_HOURS * 3_600_000).toISOString()
    const { data } = await sb
      .from('meta_snapshots')
      .select('overview,prev,prev_since,prev_until,campaigns,trend,platforms,by_objective,since,until_date,synced_at')
    ...
    return {
      ...
      prevSince: row.prev_since,
      prevUntil: row.prev_until,
      overview:  row.overview,
      prev:      row.prev,
      campaigns: row.campaigns,
      trend:     row.trend,
      platforms: data.platforms,
      byObjective: row.by_objective,
...
async function saveCache(
  ...
  prevSince: string, prevUntil: string,
  payload: Record<string, unknown>
) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const sb = getSupabase() as any
  await sb.from('meta_snapshots').upsert({
    ...
    by_objective: payload.byObjective,
    synced_at:    new Date().toISOString(),
  }, { onConflict: 'account_id,since,until_date' })
}
